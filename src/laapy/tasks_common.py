...
OP_DIRLOG = 'dirlog'
OP_FILEEXT = 'fileext'
OP_FILESAMPLE = 'filesample'
OP_FILEENV = 'fileenv'
OP_TISSHOST = 'tisshost'
OP_EXCLUDESAMPLE = 'excludesample'
OP_SETNAME = 'setname'
OP_SETID = 'setid'
OP_SETNAME_EXTEND_BY = 'setname_extend_by'
OP_SETID_EXTEND_BY = 'setid_extend_by'
OP_MMSID = 'mmsid'
OP_HOLID = 'holid'
OP_ITEMID = 'itemid'
OP_BARCODE = 'barcode'
OP_COLLECTIONID = 'collectionid'
OP_SERVICEID = 'serviceid'
OP_PORTFOLIOID = 'portfolioid'
OP_USERID = 'userid'
OP_REQUESTID = 'requestid'
OP_FEEID = 'feeid'
OP_MAXRECORDS = 'maxrecords'
OP_OFFSET = 'offset'
OP_LIMIT = 'limit'
OP_REQUEST_TYPE = 'request_type'
OP_REQUEST_SUB_TYPE = 'request_sub_type'
OP_REQUEST_DEST = 'request_dest'
OP_REQUEST_STATUS = 'request_status'
OP_DEPARTMENT = 'department'
OP_WOTYPE = 'wotype'
OP_WOSTATUS = 'wostatus'
OP_DONE = 'done'
OP_LIBRARY = 'library'
OP_LOCATION = 'location'
OP_CIRCDESK = 'circdesk'
OP_URL = 'url'
OP_URLPARAMS = 'urlparams'
OP_LOGLEVEL = 'loglevel'
OP_DBUSER = 'dbuser'
OP_DBLOG = 'dblog'
OP_DBLOGLABEL = 'dbloglabel'
OP_APICFG = 'apicfg'
OP_DRYRUN = 'dryrun'
OP_NO_DEFAULT_CONFIG = 'no_default_config'
OP_CACHE = 'cache'
OP_CODE = 'code'
OP_RANGE = 'range'
OP_FLAGS = 'flags'
""" miscellaneous flags separated by ',' which optionally can hold a value indicated by '=', eg. --flags skip_empty_barcode,create_dummy_barcode"""
FLAG_SKIP_EMPTY_BARCODE = 'skip_empty_barcode'
FLAG_CREATE_DUMMY_BARCODE = 'create_dummy_barcode'
FLAG_SUPPRESS = 'suppress'
OPTIONS = {}


def setParserArguments(parser,tasks = None,api_targets = None):
    if tasks: parser.add_argument(OP_TASK, nargs=1, help='task to be performed',choices=tasks)
    if api_targets: parser.add_argument(OP_API_TARGET, nargs=1, help='api target',choices=api_targets)
    parser.add_argument('--'+OP_FILEIN, nargs=1, required=False, help='filein', default=None)
    parser.add_argument('--'+OP_FILEOUT, nargs=1, required=False, help='fileout', default=None)
    parser.add_argument('--'+OP_DIRIN, nargs=1, required=False, help='dirin', default=None)
    parser.add_argument('--'+OP_DIROUT, nargs=1, required=False, help='dirout', default=None)
    parser.add_argument('--'+OP_DIRLOG, nargs=1, required=False, help='dirlog, directory for log files', default=None)
    parser.add_argument('--'+OP_APICFG, nargs=1, required=False, help='apicfg', default=None)
    parser.add_argument('--'+OP_SETNAME, nargs='?', help='setname')
    parser.add_argument('--'+OP_SETID, nargs='?', help='setid')
    parser.add_argument('--'+OP_SETNAME_EXTEND_BY, nargs='?', help='setname_extend_by')
    parser.add_argument('--'+OP_SETID_EXTEND_BY, nargs='?', help='setid_extend_by')
    parser.add_argument('--'+OP_MMSID, nargs='?', help='mmsid')
    parser.add_argument('--'+OP_HOLID, nargs='?', help='holid')
    parser.add_argument('--'+OP_ITEMID, nargs='?', help='itemid')
    parser.add_argument('--'+OP_COLLECTIONID, nargs='?', help='collectionid')
    parser.add_argument('--'+OP_SERVICEID, nargs='?', help='serviceid')
    parser.add_argument('--'+OP_PORTFOLIOID, nargs='?', help='portfolioid')
    parser.add_argument('--'+OP_USERID, nargs='?', help='userid')
    parser.add_argument('--'+OP_REQUESTID, nargs='?', help='requestid')
    parser.add_argument('--'+OP_FEEID, nargs='?', help='feeid')
    parser.add_argument('--'+OP_BARCODE, nargs='?', help='barcode')
    parser.add_argument('--'+OP_TISSHOST, nargs='?', help='tisshost')
    parser.add_argument('--'+OP_CODE, nargs='?', help='code')
    parser.add_argument('--'+OP_RANGE, nargs='?', help='range')
    parser.add_argument('--'+OP_MAXRECORDS, help='maxrecords', default=999999, type=int)
    parser.add_argument('--'+OP_LIMIT, help='limit', default=100, type=int)
    parser.add_argument('--'+OP_OFFSET, help='offset', default=0, type=int)
    parser.add_argument('--'+OP_FILEEXT, nargs=1, required=False, help='fileext', default=None)
    parser.add_argument('--'+OP_FILESAMPLE, nargs=1, required=False, help='filesample', default=None)
    parser.add_argument('--'+OP_EXCLUDESAMPLE, required = False, help='excludesample', action='store_true')
    parser.add_argument('--'+OP_FILEENV, nargs=1, required=False, help='fileenv', default=None)
    parser.add_argument('--'+OP_URL, nargs='?', help='url')
    parser.add_argument('--'+OP_URLPARAMS, nargs='?', help='urlparams')
    parser.add_argument('--'+OP_DBUSER, nargs='?', help='dbuser')
    parser.add_argument('--'+OP_DBLOG, required = False, help='dblog', action='store_true')
    parser.add_argument('--'+OP_DBLOGLABEL, nargs='?', help='dbloglabel')
    parser.add_argument('--'+OP_LOGLEVEL, required=False, help='loglevel', default='DEBUG')
    parser.add_argument('--'+OP_REQUEST_TYPE, required=False, help='request_type', default=None)
    parser.add_argument('--'+OP_REQUEST_SUB_TYPE, required=False, help='request_sub_type', default=None)
    parser.add_argument('--'+OP_REQUEST_DEST, required=False, help='request_dest', default=None)
    parser.add_argument('--'+OP_REQUEST_STATUS, required=False, help='request_status', default=None)
    parser.add_argument('--'+OP_LIBRARY, required=False, help='library', default=None)
    parser.add_argument('--'+OP_LOCATION, required=False, help='location', default=None)
    parser.add_argument('--'+OP_CIRCDESK, required=False, help='circdesk', default=None)
    parser.add_argument('--'+OP_DEPARTMENT, required=False, help='department', default=None)
    parser.add_argument('--'+OP_WOTYPE, required=False, help='wotype', default=None)
    parser.add_argument('--'+OP_WOSTATUS, required=False, help='wostatus', default=None)
    parser.add_argument('-'+OP_DONE, required=False, help='done', action='store_true')
    parser.add_argument('-'+OP_DRYRUN, help='dryrun', action='store_true')
    parser.add_argument('-'+OP_NO_DEFAULT_CONFIG, help='no_default_config', action='store_true')
    parser.add_argument('-'+OP_CACHE, help='cache', action='store_true')
    parser.add_argument('--'+OP_FLAGS, required=False, help='flags', default=None)
    parser.add_argument('--'+OP_FILES, required=False, help='files', default=None)

def getParserValues(parser):
    args = parser.parse_args()
    task = getattr(args,OP_TASK)[0] if hasattr(args, OP_TASK) and getattr(args,OP_TASK) else None  
    api_target = getattr(args,OP_API_TARGET)[0] if hasattr(args, OP_API_TARGET) and getattr(args,OP_API_TARGET) else None  
    OPTIONS[OP_FILEIN] = getattr(args,OP_FILEIN)[0] if hasattr(args, OP_FILEIN) and getattr(args,OP_FILEIN) else None  
    OPTIONS[OP_FILEOUT] = getattr(args,OP_FILEOUT)[0] if hasattr(args, OP_FILEOUT) and getattr(args,OP_FILEOUT) else None  
    OPTIONS[OP_DIRIN] = getattr(args,OP_DIRIN)[0] if hasattr(args, OP_DIRIN) and getattr(args,OP_DIRIN) else None  
    OPTIONS[OP_DIROUT] = getattr(args,OP_DIROUT)[0] if hasattr(args, OP_DIROUT) and getattr(args,OP_DIROUT) else None  
    OPTIONS[OP_DIRLOG] = getattr(args,OP_DIRLOG)[0] if hasattr(args, OP_DIRLOG) and getattr(args,OP_DIRLOG) else None  
    OPTIONS[OP_FILEEXT] = getattr(args,OP_FILEEXT)[0] if hasattr(args, OP_FILEEXT) and getattr(args,OP_FILEEXT) else None  
    OPTIONS[OP_FILEENV] = getattr(args,OP_FILEENV)[0] if hasattr(args, OP_FILEENV) and getattr(args,OP_FILEENV) else None  
    OPTIONS[OP_FILESAMPLE] = getattr(args,OP_FILESAMPLE)[0] if hasattr(args, OP_FILESAMPLE) and getattr(args,OP_FILESAMPLE) else None  
    OPTIONS[OP_APICFG] = getattr(args,OP_APICFG)[0] if hasattr(args, OP_APICFG) and getattr(args,OP_APICFG) else None  
    OPTIONS[OP_SETNAME] = getattr(args,OP_SETNAME,None)
    OPTIONS[OP_SETID] = getattr(args,OP_SETID,None)
    OPTIONS[OP_SETNAME_EXTEND_BY] = getattr(args,OP_SETNAME_EXTEND_BY,None)
    OPTIONS[OP_SETID_EXTEND_BY] = getattr(args,OP_SETID_EXTEND_BY,None)
    OPTIONS[OP_MMSID] = getattr(args,OP_MMSID,None)
    OPTIONS[OP_HOLID] = getattr(args,OP_HOLID,None)
    OPTIONS[OP_ITEMID] = getattr(args,OP_ITEMID,None)
    OPTIONS[OP_COLLECTIONID] = getattr(args,OP_COLLECTIONID,None)
    OPTIONS[OP_SERVICEID] = getattr(args,OP_SERVICEID,None)
    OPTIONS[OP_PORTFOLIOID] = getattr(args,OP_PORTFOLIOID,None)
    OPTIONS[OP_USERID] = getattr(args,OP_USERID,None)
    OPTIONS[OP_FEEID] = getattr(args,OP_FEEID,None)
    OPTIONS[OP_REQUESTID] = getattr(args,OP_REQUESTID,None)
    OPTIONS[OP_BARCODE] = getattr(args,OP_BARCODE,None)
    OPTIONS[OP_TISSHOST] = getattr(args,OP_TISSHOST,None)
    OPTIONS[OP_CODE] = getattr(args,OP_CODE,None)
    OPTIONS[OP_RANGE] = getattr(args,OP_RANGE,None)
    OPTIONS[OP_MAXRECORDS] = int(getattr(args,OP_MAXRECORDS))
    OPTIONS[OP_OFFSET] = int(getattr(args,OP_OFFSET))
    OPTIONS[OP_LIMIT] = int(getattr(args,OP_LIMIT))
    OPTIONS[OP_EXCLUDESAMPLE] = getattr(args,OP_EXCLUDESAMPLE,False)
    OPTIONS[OP_DBUSER] = getattr(args,OP_DBUSER,None)
    OPTIONS[OP_DBLOG] = getattr(args,OP_DBLOG,False)
    OPTIONS[OP_DBLOGLABEL] = getattr(args,OP_DBLOGLABEL,None)
    OPTIONS[OP_LOGLEVEL] = getattr(args,OP_LOGLEVEL)
    OPTIONS[OP_REQUEST_TYPE] = getattr(args,OP_REQUEST_TYPE,None)
    OPTIONS[OP_REQUEST_SUB_TYPE] = getattr(args,OP_REQUEST_SUB_TYPE,None)
    OPTIONS[OP_REQUEST_DEST] = getattr(args,OP_REQUEST_DEST,None)
    OPTIONS[OP_REQUEST_STATUS] = getattr(args,OP_REQUEST_STATUS,None)
    OPTIONS[OP_LIBRARY] = getattr(args,OP_LIBRARY,None)
    OPTIONS[OP_LOCATION] = getattr(args,OP_LOCATION,None)
    OPTIONS[OP_CIRCDESK] = getattr(args,OP_CIRCDESK,None)
    OPTIONS[OP_DEPARTMENT] = getattr(args,OP_DEPARTMENT,None)
    OPTIONS[OP_WOTYPE] = getattr(args,OP_WOTYPE,None)
    OPTIONS[OP_WOSTATUS] = getattr(args,OP_WOSTATUS,None)
    OPTIONS[OP_DONE] = getattr(args,OP_DONE,False)
    OPTIONS[OP_URL] = getattr(args,OP_URL,None)
    OPTIONS[OP_DRYRUN] = getattr(args,OP_DRYRUN,False)
    OPTIONS[OP_CACHE] = getattr(args,OP_CACHE,False)
    OPTIONS[OP_NO_DEFAULT_CONFIG] = getattr(args,OP_NO_DEFAULT_CONFIG,False)

    if OPTIONS[OP_APICFG] is None and OPTIONS[OP_NO_DEFAULT_CONFIG] == False:
        OPTIONS[OP_APICFG] = os.path.join(os.path.abspath(sys.path[0]),'api.json')

    urlparams = getattr(args,OP_URLPARAMS,None)
    if urlparams:
        OPTIONS[OP_URLPARAMS] = {}
        l = re.split(',',urlparams)
        for s in l:
            m = re.split('=',s)
            OPTIONS[OP_URLPARAMS][m[0]] = m[1] if m[1] else None

    files = getattr(args,OP_FILES,None)
    if files:
        OPTIONS[OP_FILES] = {}
        l = re.split(';',files)
        for s in l:
            m = re.split('=',s)
            OPTIONS[OP_FILES][m[0]] = m[1] if m[1] else None


    flags = getattr(args,OP_FLAGS,None)
    if flags:
        OPTIONS[OP_FLAGS] = {}
        l = re.split(',',flags)
        for s in l:
            m = re.split('=',s)
            OPTIONS[OP_FLAGS][m[0]] = m[1] if len(m) > 1 else True

    return task, api_target

def setupLogging(basefile,task=None,dirlog=None):
    warn_log = None

    if dirlog:
        if not os.path.exists(dirlog):
            warn_log = "log directory doesn't exist. using default location"

        elif not os.path.isdir(dirlog):
            warn_log = "option given as log directory exists, but isn't a directory. using default location"

    now = datetime.datetime.now()
    filename = 'log_' + basefile + '_' + task + '_' + now.strftime("%Y%m%d_%H%M%S")
    if not warn_log and dirlog:
        filename = os.path.join(dirlog,filename)
    ...
    stream_handler.setFormatter(logging.Formatter(logfmt,datefmt=datefmt))

    file_handler = logging.FileHandler(filename=filename)
    file_handler.setFormatter(logging.Formatter(logfmt,datefmt=datefmt))
...
"15.43":"Deutsche Geschichte des 19. und 20. Jahrhunderts",
...
"17.08":"Semiotik",
"17.10":"Sprache in Beziehung zu anderen Bereichen der Wissenschaft und ...",
...
"17.90":"Literatur in Beziehung zu anderen Bereichen von Wissenschaft un...",
...
"17.97":"Texte einzelner Autoren",
"17.98":"Textsammlungen",
"17.99":"Literaturwissenschaft: Sonstiges",
"18.00":"Sprache und Literatur allgemein",
...
"18.68":"Mittelindische Sprachen und Literaturen, neuindische Sprachen u...",
...
"18.93":"Indianersprachen und -literaturen, Eskimosprachen und -literatu...",
"18.94":"Afrikanische Sprachen und Literaturen",
"18.95":"Austronesische Sprachen und Literaturen, australische Sprachen ...",
...
"33.62":"Mechanische Eigenschaften, akustische Eigenschaften, thermische...",
...
"43.02":"Philosophie und Theorie der Umweltforschung und des Umweltschut...",
...
"52.01":"Geschichte des Maschinenbaus, der Energie- und Fertigungstechni...",
...
"70.03":"Methoden, Techniken und Organisation der sozialwissenschaftlich...",
...
"81.50":"Pädagogische Psychologie, Unterrichtspsychologie, Schulpsycholo...",
"81.51":"Lernprozess",
"81.52":"Lernleistung",
...
"81.59":"Pädagogische Psychologie, Unterrichtspsychologie, Schulpsycholo...",
...
"85.27":"Betriebswirtschaftliche Steuerlehre, Betriebliches Prüfungswese...",
...
"88.20":"Organisation staatlicher Einrichtungen, Management staatlicher ...",
